...
def buy_index(capital: float, logs: TextIOWrapper, api: api_wrapper):
    ...
    price_diffs = {ticker: price - start_prices[ticker] for ticker, price in prices.items()}
    top_stocks = sorted(price_diffs.items(), key=lambda x: x[1], reverse=True)[:AMOUNT_OF_STOCKS_IN_INDEX]
...
def sell_index(capital: float, logs: TextIOWrapper, api: api_wrapper, amounts: dict[str, int]):
